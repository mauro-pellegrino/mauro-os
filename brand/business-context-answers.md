# @maurojpelle — Business Context (first-person answers)
**Date:** 2026-07-01
**Use:** Copy-paste answers to the "$100k/mo entrepreneur" business-context intake. Written first person, in Mauro's voice. Companion to `operating-baseline.md` (which holds the ruthless analysis).

---

**Who do you help?**
Established agency owners, the Lorenzo and Bogdan type. Operators already running a real agency, mid six figures a month and up, who know they should be using AI to produce content and win clients but don't have the systems for it. Not beginners. Not solo freelancers hunting cheap prompts. Operators who want the machine installed.

**What 15 stories do they tell themselves to justify their failures and purchases?**
1. My work is too custom to systematize or hand to AI.
2. AI content looks generic and will cheapen my brand.
3. Only I can keep the quality high, so I have to stay in delivery.
4. Once I make the right hire, I'll finally get out of the weeds.
5. I don't have time to build systems, I'm too busy doing the work.
6. My clients pay for me specifically, so I can't step back.
7. I'll package and productize later, once things calm down.
8. More leads will fix this, when it's really conversion or offer.
9. This next tool or hire is the thing that unlocks scale.
10. Retention is bad because clients don't understand the value.
11. My market won't pay more than this.
12. I just need to post more and be more consistent.
13. I didn't start posting three years ago, so I'm cooked now.
14. Cold email and outbound is just better than content.
15. Why post at all if I can rely on referrals?

**What's your numerical / emotional goal? What do you want it to feel like?**
$5k/mo from this brand by the end of 2026. Emotionally, I want to run Growthub and my own brand at the same time, sharing what actually works and getting paid from both. A solid X and YouTube channel that's mine. Something long-lasting that I built for myself, that helps the version of me from a few years ago and others like him.

**What's the #1 thing keeping me from it?**
I don't have an offer and I'm not posting consistently. Underneath that, I hide in building systems because it feels productive, instead of shipping my own thing and putting it in front of people. I optimize what I can measure and avoid the uncomfortable work of selling.

**What's your current offer and pricing?**
None. Zero. I've floated $100 workshops or done-for-you setup of the systems I run (lead magnets, X articles, infographics), but nothing exists yet, and that price is way too low for who I actually want to sell to.

**How do people find you right now?**
Barely anywhere. X with about 200 followers and one YouTube video. No ads, no list, no newsletter. Right now I mostly ride the agency's channels. This brand has almost no discovery of its own.

**What's your monthly revenue and your revenue goal?**
This brand makes $0 today. Goal is $5k/mo by end of 2026, which for high-ticket buyers is one or two clients, not an audience. Credibility context: the agency I run, Growthub, is at about $300k/mo, at least a third of that comes from the organic accounts I manage, and I just closed a $28k deal off X.

**What's your email list size and what platform do you use?**
No list and no platform yet. I want one.

**What are 15 core beliefs about life / your business that set you apart?**
1. Systems beat random effort. Always bring it back to the repeatable process.
2. Show the work, don't just claim it. Post the actual builds and numbers.
3. Specificity is credibility. Use a real number or say nothing.
4. AI is leverage inside an operator's workflow, not a replacement for taste.
5. The best content doesn't look like content, the best ad doesn't look like an ad.
6. Speak to the sophisticated operator, never the beginner.
7. Steal the customer's exact words. Copy channels desire, it doesn't create it.
8. Every word earns its place.
9. Build in public. Real work is the content engine.
10. Never fabricate a number. Bracket it or leave it out.
11. Volume without a map is guessing with a bigger budget.
12. One source, many audiences. Repurpose the same IP across lanes.
13. Truth over flattery, even when it stings.
14. Speed over perfection. Ship and iterate.
15. Own your lane so it can't be absorbed by a bigger brand next to you.

**What tech stack do you run on?**
Almost nothing for this brand. No payment processor, no course or community platform, no CRM, no email tool. Just Claude and the systems I've built in my own repo. (The agency runs a full stack, Calendly, Tweet Hunter, Slack, tl;dv, a Next.js dashboard, but that's Growthub, not this.)

**What's your biggest bottleneck, leads, conversions, retention, or offer clarity?**
Offer clarity first, then leads. I have no offer, so nothing downstream can work. It isn't a conversion or retention problem because there's nothing yet to convert or retain.

**What have you already tried that didn't work?**
Posting and commenting on X through a VA. It doesn't work for this account. It needs my own voice and my real work, not outsourced activity.

**What does your team look like?**
Solo for this brand. I have a team and VAs at the agency, but @maurojpelle is just me, and it currently gets my leftover hours.

**How many hours per day do you actually work on the business?**
About an hour a day, 5 to 7 hours a week. And honestly a lot of that goes to building instead of shipping.

**Describe your writing style in 10 sentences.**
1. I write in plain, direct English, short declarative sentences, one idea per line.
2. No em dashes, ever.
3. No "it's not X, it's Y" contrast structures or clever reframes.
4. No "most brands" or "most people" openers.
5. Numbers are always specific and real, never vague and never made up.
6. It reads like an operator talking to a smart peer, not an essay and not a lecture.
7. My openers are driven by the material, I lead with the strongest hook the piece actually has.
8. Confident and slightly contrarian, expert to expert, no hedging.
9. I hand over the actual goods, real prompts and real steps, instead of gatekeeping.
10. No trailing summaries, no filler closings, I end on the point.

**What's the #1 thing you wish someone would just solve for you?**
Tell me exactly what to do day to day, whether to post, comment, or focus on YouTube and in what mix, so I stop guessing and just execute. Under that, package what I know into an offer I can sell without it eating the hours I don't have.

**What do you hate?**
Bad content. Bland, obviously-AI content. Lazy cringe LinkedIn posts, though bold or cringe is fine if it's actually good, the line is quality not tone. Fluff, being flattered instead of told the truth, slowness, and beginner-level positioning.
